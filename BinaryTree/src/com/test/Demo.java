...
public class Demo {
		...
		while (!queue.isEmpty()) {
			...
			if (mustHaveNoChild) {// 出现叶子节点，所以左右子树必须为null
				if (cur.left != null || cur.right != null) {
					result = false;// 只要左子树或者右子树不为空，则返回false
					break;
				}
			} else {
				// 左子树右子树都为空则继续遍历
				...
				// 左子树不为空右子树为空，则说明出现空节点，以后必须为空节点，继续遍历左子树
				if (cur.left != null && cur.right == null) {
					mustHaveNoChild = true;
					queue.add(cur.left);
				}
				// 左子树为空，右子树不为空说明不是完全二叉树，返回false
				if (cur.left == null && cur.right != null) {
					result = false;
					break;
				} // 左子树右子树都为空，则说明没有子节点
	...
	/**
	 * 14. 由前序遍历序列和中序遍历序列重建二叉树（递归） 感觉这篇是讲的最为清晰的:
	...
	public static TreeNode rebuildBinaryTreeRec(List<Integer> PreOrder, List<Integer> InOrder) {
		TreeNode root = null;
		List<Integer> leftPredOrder;// 先序遍历左子树
		List<Integer> lefInOrder;// 中序遍历左子树
		List<Integer> rightPredOrder;// 先序遍历右子树
		List<Integer> rightInOrder;// 中序遍历右子树
		int inorderPos;
		int preorderPos;
		if (PreOrder.size() != 0 && InOrder.size() != 0) {
			root = new TreeNode(PreOrder.get(0));// 先序遍历中根节点为第一个
			inorderPos = InOrder.indexOf(PreOrder.get(0));// 找出中序遍历中左右子树的分割点
			lefInOrder = InOrder.subList(0, inorderPos);
			rightInOrder = InOrder.subList(inorderPos + 1, InOrder.size());
			preorderPos = lefInOrder.size();// 先序遍历中左右子树的分割点，就是中序遍历中左子树的大小
			leftPredOrder = PreOrder.subList(1, preorderPos);
			rightPredOrder = PreOrder.subList(preorderPos + 1, PreOrder.size());
			root.left = rebuildBinaryTreeRec(leftPredOrder, lefInOrder);// 左子树就是两者左区间形成的
			root.right = rebuildBinaryTreeRec(rightPredOrder, rightInOrder);// 右子树就是两者又区间形成的
		}
		return root;
	}
}
